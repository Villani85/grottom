...
export async function GET(req: Request) {
  ...
  const path = url.searchParams.get("path") ?? "videos/demo.mp4";
  ...
  if (isDemoMode()) {
    return NextResponse.json({ ok: true, url: `https://example.com/mock-signed-url?path=${encodeURIComponent(path)}`, demo: true });
  }
